<!doctype html>
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        <meta charset="utf-8">
        <meta name="viewport" content="width=device-width, initial-scale=1">
        <meta name="viewport"
              content="width=device-width, user-scalable=no, initial-scale=1.0, maximum-scale=1.0, minimum-scale=1.0">
        <meta http-equiv="X-UA-Compatible" content="ie=edge">
        <title>Machine Vision</title>
        <link rel="stylesheet" href="/css/icons.css">
        <link rel="stylesheet" href="/css/slick.css">
        <link rel="stylesheet" href="/css/style.css">
    </head>
    <body id="mainpage">

    <header class="default">
        <div class="wrapper">

            <a href="/" class="logo">
                <img src="/images/logo.png" alt="logo.png">
            </a>

            <ul class="menu__list">
                <li class="menu__item"><a href="#mainpage">Главная</a></li>
                <li class="menu__item"><a href="#catalog">Оборудование</a></li>
                <li class="menu__item"><a href="#contacts">Контакты</a></li>
            </ul>

            <a href="tel:+375296933037" class="phone">+375 (29) 693-30-37</a>

            <a href="#" class="mobile__link"><i class="icon-menu"></i></a>

        </div>
    </header>

    <div class="shadow"></div>

    <div class="mobile__menu">
        <a href="#" class="mobile__close"><i class="icon-cancel"></i></a>

        <ul class="menu__list">
            <li class="menu__item"><a href="#mainpage">Главная</a></li>
            <li class="menu__item"><a href="#catalog">Оборудование</a></li>
            <li class="menu__item"><a href="#contacts">Контакты</a></li>
        </ul>

    </div>

    <main class="default">

        <div class="slide__list">

            <div class="slide__item">
                <div class="slide__img">
                    <img class="img__lazy" data-src="/images/slider/4.jpg" alt="4.jpg">
                </div>

                <div class="slide__content">
                    <div class="slide__content_left">
                        <p class="slide__content_header">Лаборатория машинного зрения</p>
                    </div>
                    <div class="slide__content_right">
                        <p class="slide__content_desc">Оптоэлектронные приборы для точного измерения геометрических параметров, применяемые с целью автоматизации в промышленной сфере </p>
                        <a href="#" class="link">заказать звонок</a>
                    </div>
                </div>

                <div class="slide__next">
                    <i class="icon-right"></i>
                </div>
            </div>

            <div class="slide__item">
                <div class="slide__img">
                    <img class="img__lazy" data-src="/images/slider/1.png" alt="1.png">
                </div>

                <div class="slide__content">
                    <div class="slide__content_left">
                        <p class="slide__content_header">Лаборатория машинного зрения</p>
                    </div>
                    <div class="slide__content_right">
                        <p class="slide__content_desc">Оптоэлектронные приборы для точного измерения геометрических параметров, применяемые с целью автоматизации в промышленной сфере </p>
                        <a href="#" class="link">заказать звонок</a>
                    </div>
                </div>

                <div class="slide__next">
                    <i class="icon-right"></i>
                </div>
            </div>

            <div class="slide__item">
                <div class="slide__img">
                    <img class="img__lazy" data-src="/images/slider/2.png" alt="2.png">
                </div>

                <div class="slide__content">
                    <div class="slide__content_left">
                        <p class="slide__content_header">Лаборатория машинного зрения</p>
                    </div>
                    <div class="slide__content_right">
                        <p class="slide__content_desc">Оптоэлектронные приборы для точного измерения геометрических параметров, применяемые с целью автоматизации в промышленной сфере </p>
                        <a href="#" class="link">заказать звонок</a>
                    </div>
                </div>

                <div class="slide__next">
                    <i class="icon-right"></i>
                </div>
            </div>
            <div class="slide__item">
                <div class="slide__img">
                    <img class="img__lazy" data-src="/images/slider/3.jpg" alt="3.jpg">
                </div>

                <div class="slide__content">
                    <div class="slide__content_left">
                        <p class="slide__content_header">Лаборатория машинного зрения</p>
                    </div>
                    <div class="slide__content_right">
                        <p class="slide__content_desc">Оптоэлектронные приборы для точного измерения геометрических параметров, применяемые с целью автоматизации в промышленной сфере </p>
                        <a href="#" class="link">заказать звонок</a>
                    </div>
                </div>

                <div class="slide__next">
                    <i class="icon-right"></i>
                </div>
            </div>


        </div>

        <div class="top__desc">
            <p class="top__desc_text">Мы предлагаем комплексные решения для контроля и автоматизации производственных задач, начиная от индивидуальных датчиков и преобразователей до многофункциональных систем измерения и управления</p>
        </div>

        <div class="catalog__list">
            <div class="wrapper">
                <p class="catalog__title" id="catalog">Оборудование</p>
            </div>
            <div class="wrapper">
                <div class="catalog__item">

                    <div class="catalog__item_content">
                        <p class="catalog__item_number">01</p>
                        <p class="catalog__item_title">Лазерные сканеры AN-x2TL</p>
                        <p class="catalog__item_desc">Лазерные сканеры предназначены для бесконтактного измерения и сканирования профиля поверхности, положения, смещения, размеров, порядка сортировки, трехмерного сканирования, высокоточного измерения геометрических параметров и положения объекта.</p>
                        <a href="#" class="link__primary"><i class="icon-download-alt"></i>Характеристики</a>
                    </div>
                    <div class="catalog__item_img">
                        <img class="img__lazy" data-src="/images/catalog/1.png" alt="1.png">
                    </div>
                </div>
                <div class="catalog__item">

                    <div class="catalog__item_content">
                        <p class="catalog__item_number">02</p>
                        <p class="catalog__item_title">Интеллектуальное машинное зрение</p>
                        <p class="catalog__item_desc">Интеллектуальное машинное зрение для промышленного сектора. Возможности контроля
                            и распознавания объектов, качества покрытия, однородности структуры материала, количества и размера.</p>

                    </div>
                    <div class="catalog__item_img">
                        <img class="img__lazy" data-src="/images/catalog/2.png" alt="1.png">
                    </div>
                </div>
                <div class="catalog__item">

                    <div class="catalog__item_content">
                        <p class="catalog__item_number">03</p>
                        <p class="catalog__item_title">Триангуляционный дальномер АН-11ТЛ</p>
                        <p class="catalog__item_desc">Триангуляционный дальномер АН-11ТЛ предназначен для высокоточного бесконтактного измерения расстояния до объекта, определения смещения относительно измерителя, измерения минимального и максимального расстояния, определения геометрии поверхности.</p>
                        <a href="#" class="link__primary"><i class="icon-download-alt"></i>Характеристики</a>
                    </div>
                    <div class="catalog__item_img">
                        <img class="img__lazy" data-src="/images/catalog/3.png" alt="1.png">
                    </div>
                </div>
                <div class="catalog__item">

                    <div class="catalog__item_content">
                        <p class="catalog__item_number">04</p>
                        <p class="catalog__item_title">Лазерные модули</p>
                        <p class="catalog__item_desc">Лазерные модули представляют собой оптоэлектронные устройства на основе полупроводниковых лазерных
                            диодов. Области применения: наука, медицина, геодезия,  строительство, деревообработка, металлообработка, робототехника, спектроскопия, печать с высоким разрешением, машинное зрение и другие.</p>
                        <a href="#" class="link__primary"><i class="icon-download-alt"></i>Характеристики</a>
                    </div>
                    <div class="catalog__item_img">
                        <img class="img__lazy" data-src="/images/catalog/4.png" alt="1.png">
                    </div>
                </div>
                <div class="catalog__item">

                    <div class="catalog__item_content">
                        <p class="catalog__item_number">05</p>
                        <p class="catalog__item_title">Оптический микрометр АН-21СМ</p>
                        <p class="catalog__item_desc">Оптический микрометр АН-21СМ позволяет измерять геометрические параметры и положение объекта в высокоточном режиме, измерять зазоры, положение, геометрические размеры в поперечном сечении и средний диаметр объекта, контролировать и устранять неисправности готовых деталей.</p>
                        <a href="#" class="link__primary"><i class="icon-download-alt"></i>Характеристики</a>
                    </div>
                    <div class="catalog__item_img">
                        <img class="img__lazy" data-src="/images/catalog/5.png" alt="1.png">
                    </div>
                </div>
                <div class="catalog__item">

                    <div class="catalog__item_content">
                        <p class="catalog__item_number">06</p>
                        <p class="catalog__item_title">Сканер внутренних диаметров труб, наконечников, отверстий АН-31RTL</p>
                        <p class="catalog__item_desc">Сканер АН-31РТЛ предназначен для бесконтактного измерения внутренней поверхности и внутреннего диаметра валов, цилиндрических и конических труб, отверстий. Сканер также позволяет находить дефекты поверхности, реконструировать профиль сечения, создавать трехмерную модель объекта.</p>
                        <a href="#" class="link__primary"><i class="icon-download-alt"></i>Характеристики</a>
                    </div>
                    <div class="catalog__item_img">
                        <img class="img__lazy" data-src="/images/catalog/6.png" alt="1.png">
                    </div>
                </div>
            </div>


        </div>

        <div class="form__container">
            <div class="wrapper">
                <form action="" class="form__wrapper" method="post" >
                    <p class="form__wrapper_title" id="contacts">Оставьте заявку</p>
                    <p class="form__wrapper_sub_title">на обратный звонок</p>

                    <label class="form__wrapper_line" for="name">
                        <input class="input" type="text" id="name" name="name">
                        <span>Ваше имя</span>
                    </label>

                    <label class="form__wrapper_line" for="phone">
                        <input class="input" type="text" id="phone" name="phone">
                        <span>Ваш телефон</span>
                    </label>

                    <label class="form__wrapper_line" for="message">
                        <textarea class="textarea" name="message" id="message"></textarea>
                        <span>Ваше сообщение</span>
                    </label>
                    <button class="link">отправить заявку</button>
                </form>
            </div>
        </div>

    </main>

    <footer class="default">
        <div class="wrapper">

            <a class="logo" href="/"><img class="img__lazy" data-src="/images/logo.png" alt="logo.png"></a>


            <div class="footer__menu">
                <ul class="menu__list">
                    <li class="menu__item"><a href="#mainpage">Главная</a></li>
                    <li class="menu__item"><a href="#catalog">Оборудование</a></li>
                    <li class="menu__item"><a href="#contacts">Контакты</a></li>
                </ul>
                <p class="copyright">© Все права защищены</p>
            </div>


            <div class="footer__contacts">
                <a href="tel:+375296933037" class="footer__phone">+375 (29) 693-30-37</a>
                <a href="#" class="footer__address">г. Минск, ул. Петруся<br>Бровки, д.30, ком. 33</a>
            </div>

        </div>
    </footer>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/jquery/3.4.1/jquery.min.js"></script>
    <script src="/js/lazy.js"></script>
    <script src="/js/slick.min.js"></script>
    <script src="/js/main.js"></script>
    </body>
</html>
